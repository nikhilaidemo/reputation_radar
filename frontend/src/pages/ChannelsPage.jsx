...
const ChannelsPage = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-gray-100">Channels View</h1>

      <div className="bg-gray-800 p-6 rounded-lg shadow-md">
        <label htmlFor="source-select" className="block text-sm font-medium text-gray-300 mb-2">
          Select Channel:
        </label>
        <select
          id="source-select"
          className="mt-1 block w-full md:w-1/3 py-2 px-3 border border-gray-600 bg-gray-700 text-gray-100 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              ...
              <ul className="divide-y divide-gray-700">
                {getPostsForSelectedSource().map((post) => (
                  <li key={post.id} className="py-3">
                    ...
                    {post.url && (
                      <a href={post.url} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-400 hover:text-indigo-300">
                        Read more
                      </a>
                    )}
                  </li>
